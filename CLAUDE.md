...
- **market_comps build unified (Track B Stage 1 PR A).** Formerly two competing writers (`autoBootstrapCompSet` filtered top-8 on import; `buildCompSet` unfiltered top-15 on daily `market_sync.py` refresh) clobbered each other via DELETE-then-INSERT. Now a single canonical `buildFilteredCompSet` in `src/lib/airroi/compsets.ts` serves both callers (`/api/properties/import-from-url`, `/api/market/refresh`, `/api/market/comps`). Legacy `buildCompSet` + `storeCompSet` deleted. Filter policy: bedrooms match, ±20% of property median rate (if known), 2km radius, top 8 by occupancy.
