...
### Known fidelity gaps (staging vs production)

- **DRIFT-3: Staging missing Supabase role grants.** Production has platform-provisioned grants (anon, authenticated, service_role roles have USAGE on public schema and table-level access); staging doesn't. Surfaced during Milestone 1 Phase 2 RLS round-trip testing, worked around via transactional GRANT/ROLLBACK. Not blocking current work. Eventual fix path: investigate whether grants are migration-authorable or are a tier-related Supabase platform limitation. See Milestone 1 rollout report DRIFT-3 section (`docs/architecture/agent-loop-v1-milestone-1-rollout-report.md`).
...
- **Pre-activation gate for `messaging_executor`** — Before activating `messaging_executor` in production, verify the INSERT path sets `actor_kind='agent'` and `actor_id=NULL` on every message it generates. The Milestone 1 back-population only ran once against existing rows; new rows require explicit attribution at INSERT time. Without this, executor-generated messages will be attributed as host-authored (NULL actor_kind defaults to falling outside the voice extraction filter, but a missing or wrong attribution pollutes the substrate either way). See `docs/architecture/agent-loop-v1-milestone-1-rollout-report.md` for the back-population rationale.
- **`messages_pre_milestone1_snapshot` rollback-safety table** — Created during Milestone 1 production rollout (2026-05-02) as a 90-row snapshot of `messages` before back-population. Schedule: `DROP` this table on or after 2026-05-09 (7-day observation window per the Milestone 1 test plan §A4). It serves no functional purpose post-window; leaving it in production is mild schema clutter. See rollout report Phase 5 for context.
...
- **J3 LLM-judge fail-mode binding contract** (M12 Phase B onward) — J3 gates host-to-guest output at 3 call sites: `/api/messages/draft`, `/api/reviews/respond/[reviewId]`, `/api/reviews/generate-guest-review/[reviewId]`. Default fail-mode is FAIL-OPEN-WITH-FLAG (judge-infra failure → ship + envelope flag; verdict=fail → annotate, host decides via `PendingDraftBubble.tsx:60` generic dispatch). **This default is VALID ONLY while host-approval gates the send path.** The judge is advisory; host approval IS the gate. **The moment any call-site auto-sends without host approval (messaging_executor activation; auto-approve UI mode), the judge becomes the ONLY gate between bad output and the guest, and fail-open becomes dangerous.** Any such auto-send call-site MUST flip fail-mode via the per-call-site policy-override hook in `src/lib/agent/judge/apply-output-judges.ts` to fail-closed-or-stricter BEFORE the auto-send activates. Same shape as the M2 auto-apply / host-revert forward-contract above. See `~/koast-vault/milestones/M12/items/phase-b-phase-1-stop.md` §3.3 for the contract framing.
- **Private/Aggregate firewall contract** (M13 Phase 1.A; values-load-bearing per Belief 3 host-asset commitment) — All cross-host reads on `pricing_outcomes`, `memory_facts`, `voice_patterns`, `host_action_patterns`, `agent_audit_log`, `agent_conversations`, and `host_surface_telemetry` MUST pass through an anonymization view (when built). Zero identifying fields (host_id / property_id / user_id / agent_conversations.id / session_id) may surface in any cross-host aggregation, statistic, ML feature, training set, or dataset. At Phase 1.A the aggregate VIEW does NOT exist; ANY code that needs cross-host stats MUST be flagged at architectural-review for the future anonymization layer. **No exception.** The firewall is the moat the values commitment buys (host's accumulated knowledge is the host's asset). Same locus-of-enforcement pattern as the J3 fail-open binding contract above (CLAUDE.md is the source of truth; substrate respects it; future amendments amend here first). Forward-trigger: when the cross-host aggregate layer ships (M14+ or later), the anonymization VIEW must be authored + this contract amended to point at it as the enforcement point. See `~/koast-vault/milestones/M13/M13-phase-1-architecture-stop.md` §X.5 + `~/koast-vault/milestones/M13/M13-phase-1A-ultraplan.md` §6.9.A8 for context.
