 ...
 *     - If substrate returns mode='allow': dispatcher invokes the
 *       handler, then calls updateAuditOutcome on resolution.
 *     - If mode='block' or 'require_confirmation': dispatcher returns
 *       a ToolError with the substrate's reason and resolves the
 *       audit row to 'failed' with the gate_blocked reason.
 *
 * Either way, the audit row exists by the time the handler runs (or
 * fails to run); the dispatcher always returns a ToolCallResult that
 * carries the audit_log_id (or null in the rare case where audit
 * writing itself failed before the handler could run).
 */
...
export async function dispatchToolCall(
  ...
  // 3b. If substrate refused, resolve audit and return error
  if (gatedMode !== "allow") {
    await updateAuditOutcome(auditLogId, "failed", {
      latency_ms: Date.now() - startedAt,
      error_message: `gate_${gatedMode === "blocked" ? "blocked" : "confirmation_required"}: ${gatedReason}`,
    });
    return {
      ok: false,
      error: makeError(
        gatedMode === "blocked" ? "gate_blocked" : "confirmation_required",
        gatedReason,
      ),
