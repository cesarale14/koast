...
export async function POST(request: NextRequest) {
  try {
    ...
    const recentChecksResult = await db.select({ id: revenueChecks.id })
      .from(revenueChecks)
      .where(and(eq(revenueChecks.ipAddress, ip), gte(revenueChecks.createdAt, hourAgo)));

    if (recentChecksResult.length > 10) {
    ...
    try {
      marketSummary = await airroi.getMarketSummary({ country: market.country, region: market.region, locality: market.locality });
    } catch { /* */ }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let comps: any[] = [];
    try {
      const compResult = await airroi.getComparables(coords.lat, coords.lng, bedrooms, Math.max(1, Math.floor(bedrooms * 0.75)), bedrooms * 2 + 2);
    ...
    const suggestedRate = Math.round(compMedian * 0.95);
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const dowFactor = dow === 5 || dow === 6 ? 1.18 : dow === 5 ? 1.10 : dow === 0 ? 1.05 : 0.95;
    ...
    const result = {
      ...
      your_rate: current_rate, market_adr: Math.round(marketAdr), comp_median: Math.round(compMedian),
      top_performers: Math.round(topPerformers), suggested_rate: suggestedRate, percentile,
      annual_opportunity: annualOpportunity, market_occupancy: Math.round(marketOcc * 100),
      market_revpar: Math.round(marketRevpar), active_listings: Math.round(activeListings),
      comp_count: comps.length, comp_preview: compPreview, rate_preview: ratePreview,
    };

    await db.insert(revenueChecks).values({
      ipAddress: ip, address: address ?? null, city: city ?? market.locality,
      state: state ?? market.region, bedrooms, currentRate: String(current_rate), resultJson: result,
    });
