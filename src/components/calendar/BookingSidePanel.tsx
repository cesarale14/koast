    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-96 bg-white shadow-xl z-50 overflow-y-auto animate-slide-in">
        ...
        <div className="p-6 space-y-6">
          {/* Guest info */}
          <div>
            <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-3">Guest</h3>
            ...
            {booking.guest_phone && (
              <p className="text-sm text-gray-500 mt-0.5">{booking.guest_phone}</p>
            )}
          </div>

          {/* Badges */}
          <div className="flex items-center gap-2">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${platformColors[booking.platform] ?? "bg-gray-100 text-gray-600"}`}>
              {booking.platform}
            </span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm font-medium text-gray-900">{formatDate(booking.check_in)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400 mb-1">Check-out</p>
              <p className="text-sm font-medium text-gray-900">{formatDate(booking.check_out)}</p>
            </div>
            <div>
          ...
          <div className="space-y-2 pt-2">
            ...
            <Link
              href="/turnover"
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
