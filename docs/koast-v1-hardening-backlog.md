...
### H3.3 — three apply routes still carry an inline per-channel dispatch loop — ✅ DONE (P4.3 + P4-closeout)
**CLOSED 2026-06-12.** All three apply routes now push through the one canonical
writer `applyOtaRestrictions`; the inline BDC→safe / non-BDC→direct loops are gone.
- `/api/calendar/rates/apply` (P4.3, `targetChannels`), `/api/pricing/apply`
  (closeout, `capturePriorState` + M2 revert preserved), `/api/channels/rates` POST
  (closeout, `targetChannel`) — each migrated behind a characterization test written
  FIRST; behavior-preserving (response + audit shapes intact). One known tightening:
  channels/rates no longer pushes to `pending_authorization` channels (saved locally,
  not pushed) — documented in-code. Live `buildSafeBdcRestrictions` callers remaining
  are out-of-scope surfaces (preview/commit/push/activate/base-rate/revert), not apply.
